...
class Order:
    ...
    def format_as_dict(self):
        elements_as_dict = {f"client_name": self.client_first_name, "client_last_name": self.client_last_name}
        for index, element in enumerate(self._order_elements):
            elements_as_dict.update(
                {f"product_{index}": {f"product_name": f"{element.product}", f"quantity": f"{element.quantity}"}})
        return elements_as_dict
